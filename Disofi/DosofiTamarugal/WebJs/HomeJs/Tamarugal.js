...
$(document).ready(function () {
    ...
    $("#btnGrabarDatosDespacho").click(function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data > 1) {
                    alert("Se creado Despacho N°." + data);

                    //  $("#btnImprimir").Show();   
                    $("#btnGrabarDatosDespacho").hide();
                    $('#btnImprimir').show();
                }
                if (data == 2) {
                    alert("No se puede grabar el dato en la BD, intentalo nuevamente.");
    ...
    $("#btnGrabarRecepcionGuia").click(function () {
        ...
        var vueltas = $("#txtVueltaRecep").val();
        var vueltas1 = $("#txtVueltaSoli").val();
        var vueltas2 = $("#txtVueltaHecha").val();
        var estadoRecpcion ="Pendiente";
        var totalVuelta = parseInt(vueltas) + parseInt(vueltas1);



        if ((totalVuelta) > parseInt(vueltas2)) {
            alert("Esta intentando recepcionar mas vueltas que las solicitadas.");
        }
        else {


            if (totalVuelta == vueltas2) {
                estadoRecpcion = "RECEPCIONADO";
            }
            ...
            $.ajax({
                ...
                data: {
                    ...
                    _Comentario: $("#txtComentario").val(),
                    _NumeroPedido: $("#txtNroPedido").val(),
                    _VueltasCompletadas: $("#txtVueltaRecep").val(),
                    _Estado: estadoRecpcion,
                    _IdDespacho: $("#txtDespacho").val(),
                },
                async: true,
                success: function (data) {
                    if (data == 1) {
                    //    $("#btnCerrarModal").click();
                        alert("El Ingreso se ha realizado sin problemas, Ahora puede ir a rendir los datos.");
                        

                        $("#btnGrabarRecepcionGuia").hide();
                        $('#btnRendir').show();
...
function BuscarDatosProyeccion(
    ...
    $.ajax({
        type: 'GET',
        url: "DetalleServicioTamarugalId2",
        data: {
            _idguia: _idGuia,
...
function RecepcionGuieExp(
    ...
    $("#txtServicio").val(_Servicio);
    $("#txtDespacho").val(IdDespacho);
    $("#txtVueltaHecha").val(_Vueltas);
    $("#txtVueltaSoli").val(_VueltaCompletadas); 
